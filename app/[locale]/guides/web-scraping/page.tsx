...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'webScraping' });
  return {
    title: t('metaTitle'),
    description: t('metaDesc'),
    alternates: {
      canonical: `/${locale}/guides/web-scraping`,
      languages: {
        en: '/en/guides/web-scraping',
        zh: '/zh/guides/web-scraping',
      },
    },
  };
}
...
export default async function WebScrapingGuidePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations('webScraping');
    ...
    <article className="container-page py-10 sm:py-14 max-w-3xl">
      ...
      <header className="mt-4">
        <div className="flex items-center gap-2">
          ...
          <Badge tone="muted">{t('badgeForBeginners')}</Badge>
        </div>
