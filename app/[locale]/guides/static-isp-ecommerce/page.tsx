...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'staticIsp' });
  return {
    title: t('metaTitle'),
    description: t('metaDesc'),
    alternates: {
      canonical: `/${locale}/guides/static-isp-ecommerce`,
      languages: {
        en: '/en/guides/static-isp-ecommerce',
        zh: '/zh/guides/static-isp-ecommerce',
      },
    },
  };
}
...
export default async function StaticIspGuidePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations('staticIsp');
    ...
    <article className="container-page py-10 sm:py-14 max-w-3xl">
      ...
      <header className="mt-4">
        <div className="flex items-center gap-2">
          ...
          <Badge tone="muted">{t('badgeForEcommerce')}</Badge>
        </div>
