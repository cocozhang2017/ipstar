...
import { IpGeoForm } from '@/components/tools/ip-geo-form';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'ipGeo' });
  return {
    title: t('metaTitle'),
    description: t('metaDesc'),
    alternates: {
      canonical: `/${locale}/tools/ip-geo`,
      languages: {
        en: '/en/tools/ip-geo',
        zh: '/zh/tools/ip-geo',
      },
...
export default async function IpGeoPage({
  params,
  searchParams,
}: {
  params: Promise<{ locale: string }>;
  ...
  const t = await getTranslations('ipGeo');
    ...
    <div className="animate-fade-in">
      <section className="relative overflow-hidden">
        <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
          <div className="blob blob-accent absolute -top-20 right-10 h-64 w-64 animate-float-y" />
          <div
            className="blob blob-highlight absolute top-0 left-0 h-56 w-56 animate-float-y"
            ...
            <div className="card-hero rounded-2xl p-6 sm:p-8">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                ...
                <div className="hidden sm:flex items-center gap-4">
                  ...
                  >
                    <Globe2 className="h-8 w-8 text-accent-strong dark:text-accent" strokeWidth={2} />
                  </div>
                  <div
                    className={cn(
                      'relative h-16 w-16 rounded-2xl flex items-center justify-center',
                  ...
                  >
                    <MapPin className="h-8 w-8 text-highlight" strokeWidth={2} />
                  </div>
                </div>
              </div>
...
async function InitialIpDecorator({
  ...
  return <IpGeoForm initialIp={sp.ip} />;
}
