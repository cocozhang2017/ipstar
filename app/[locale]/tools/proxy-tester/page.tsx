...
import { ProxyTesterForm } from '@/components/tools/proxy-tester-form';
import { Zap, Server, Sparkles, ArrowRight } from 'lucide-react';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'proxyTester' });
  return {
    title: t('metaTitle'),
    description: t('metaDesc'),
    alternates: {
      canonical: `/${locale}/tools/proxy-tester`,
      languages: {
        en: '/en/tools/proxy-tester',
        zh: '/zh/tools/proxy-tester',
      },
    },
  };
}

export default async function ProxyTesterPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations('proxyTester');
    ...
    <div className="animate-fade-in">
      <section className="relative overflow-hidden">
        <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
          <div className="blob blob-highlight absolute -top-20 left-10 h-64 w-64 animate-float-y" />
          <div
            className="blob blob-accent absolute top-0 right-0 h-56 w-56 animate-float-y"
                ...
                <div className="hidden sm:flex items-center gap-4">
                  <div
                    className={cn(
                      ...
                      'border border-highlight/30 bg-highlight/10',
                      'shadow-[inset_0_1px_0_rgba(255,255,255,0.5),0_12px_28px_-12px_hsl(var(--highlight)/0.4)]',
                    )}
                  >
                    <Zap className="h-8 w-8 text-highlight" strokeWidth={2} />
                  </div>
                  <div
                    className={cn(
                      'relative h-16 w-16 rounded-2xl flex items-center justify-center',
                      'border border-accent/30 bg-accent/10',
                      'shadow-[inset_0_1px_0_rgba(255,255,255,0.5),0_12px_28px_-12px_hsl(var(--accent)/0.4)]',
                    )}
                  >
                    <Server className="h-8 w-8 text-accent-strong dark:text-accent" strokeWidth={2} />
                  </div>
                </div>
              </div>
